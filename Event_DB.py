...
class Event_DB:
    ...
    def createPlannedCallList(self, event_id):
        def execute(cursor):
            query = f"Select Game, Timespan, event_plan, week From Event Where EventId = '{event_id}';"
            df = self.queried_df(cursor, query)
            game, timespan, event_plan, week = tuple(df[['game', 'timespan', 'event_plan', 'week']].iloc[0])
            create_stmt = f"""
            CREATE VIEW Planned_Call_List_{event_id} AS
            ...
            where person_completedepa.eventplanid = '{event_plan}'
            and (
                    (
                        person_completedepa.submitted_epa = true and 
                        person_eventplan_timespan.timespan = '{timespan}' and 
                        person_eventplan_timespan.week = '{week}' and 
                        person_eventplan_timespan.eventplanid = '{event_plan}'
                    ) or (
                        person_completedepa.submitted_epa = false and
                        (redeem or (eventdue and invitedue)) and 
                        person.name != 'Ian Kessler' and status = 'Active' and
                        person_games.gamesid = '{game}' and person_timespan.timespan = '{timespan}'
                    )
                )
    ...
    def getPlannedCallList(self, event_id):
        def execute(cursor):
            df = self.queried_df(cursor, f"SELECT * FROM Planned_CALL_LIST_{event_id}")
            checkboxes = df.shape[0] * [{col: False for col in ['Invited', 'Going',
                                                                'Plus One', 'Waiting', 'Decline', 'Flaked']}]
            appended_df = pd.concat([df['name'], pd.DataFrame(checkboxes), df.loc[:, df.columns != 'name']], axis=1)
            appended_df.to_csv("\\".join([os.getcwd(), 'call_lists', f"planned_call_list_{event_id}.csv"]), index=False)
        return execute
